function [tfGood, cellMesh] = spotNorm_loci( trackList, phaseList, num, ...
    cellList, cellListNAll, goodCellFlag, imgSaveFlag)
%{
Author: Yu-Huan Wang (Kim Lab at UIUC) - yuhuanw2@illinois.edu
    Creation date: 12/21/2025
    Last edited date: 12/21/2025
...
% global checkFlag img
    ...
    meshPath = trackList( num).folder;
    imgPath = fullfile( meshPath, 'phase');
    imgName = phaseList( num).name;
    cellListN = cellListNAll( num);

    % get strain & Date information from the folder
    % [ folderName, fileName, expDate, strain, extraName] = getExpInfo( meshPath, imgName);
    [~, folderName] = fileparts( meshPath);
    fileName = [ folderName ' ' imgName(1:end-4)];      % folderName + imgName as '251004-SK727 epi001'
    ...
    path = fullfile( trackList( num).folder, trackList(num).name, 'TrackingPackage', 'tracks');
    name = 'Channel_1_tracking_result.mat';    load( fullfile( path, name), 'tracksFinal')        


%% 2: Assign cell numbers to tracks (tf.cellNumber = tracksCell)
    
    % Set up the cell mesh information (contains everything about cell mesh)
    cellMesh = setCellMesh( cellList, cellListN, num);    
    
    % Mark the cells as isolated good (1), or not isolated bad (0)
    cellIsolated = markIsoCells( goodCellFlag, cellListN, cellMesh, imgPath, imgName);
    ...
    for i = 1: nTracks
        ...
        newCoords = reshape( tracksFinal(i).tracksCoordAmpCG, 8, []); 
        % extract tracksFinal info and save into separate fields
        % [x, y, z, amp, std_x, std_y, std_z, std_amp]
        tracksFinal(i).traj = newCoords( 1:2,:)';   % x & y coordiate in the unit of "pixels"        
        tracksFinal(i).amp = newCoords( 4,:)';      % signal amplitude, normalized by bit depth
        tracksFinal(i).std = sqrt( sum( newCoords( 5:6,:).^2))';        % unit: pixel
        tracksFinal(i).frame = tracksFinal(i).seqOfEvents( [1 2], 1);   % frame: [start; end]
...
%% 3: Plot & save overlay of cell image, cell outline, tracks

    close
    % load greyscale tiff images
    img = imread( fullfile( imgPath, imgName)); imshow(img, []); set( gcf, 'Position', [1100 200 900 900]);  hold on

    tracksGood = cell2mat( { tracksFinal( goodTracks).traj}'); % inside cells
    tracksBad = cell2mat( { tracksFinal( ~goodTracks).traj}'); % outside cells
    
    % plot tracks
    if ~isempty( tracksGood)
        plot( tracksGood(:,1), tracksGood(:,2) , 'r.'); % red for tracks inside cells
    end
    if ~isempty( tracksBad)
        plot( tracksBad(:,1), tracksBad(:,2) , 'c.'); % cyan for bad tracks inside cells
    end
    ...
    for Cell = 1 : cellListN
        meshOut = cellMesh( Cell).meshOut;
        if ~isempty( meshOut)
            plot( meshOut(:,1), meshOut(:,2), 'g-', 'LineWidth', 0.6)
            text( ( meshOut(10, 1)- 5), ( meshOut(10, 2)+ 5), num2str(Cell),...
                'Color', 'g', 'FontSize', 12) %, 'FontWeight', 'bold')
        end
    end    
    text(5,13, fileName, 'Color', 'w', 'FontSize', 16)   % as '251004-SK727 epi001'
    if imgSaveFlag
        exportgraphics( gca, fullfile( meshPath, 'Mesh & Tracks Image', [imgName(1:end-4) '_TracksMesh.png']), 'Resolution', 150)
...
%% Step 5: Find the normalized positions of spots within the cell and save to tfGood  
    ...
    % plot the cell mesh (all segements) overlay with PC image (optional, for checking)
%     plotMesh( meshPath, imgName, goodCells, cellMesh);

    % find track position inside cells, add field 'spotPosNorm'
    %      & field 'spotPosInCell' (optional)
    tfGood = getNormPos( tfGood, cellMesh, checkFlag, img);
...
%% Functions
    ...
    % function [ folderName, fileName, expDate, strain, extraName] = getExpInfo( meshPath, imgName)
    ...
    % 
    %     % Num = imgName(3:5);                               % follow rules of naming: BF001, BF021
    %     tmp = split( meshPath, '\');
    %     folderName = tmp{ end};                             % FolderName: '230712-SK187 Suc'
    %     fileName = [ folderName ' ' imgName(1:end-4)];      % folderName + imgName as '230712-SK187 Suc BF001'
    % 
    ...
    % 
    %     % imgName = [imgName(1:end-4) '.tif'];                % m.mat --> .tif
    %     disp([' -- We are looking at ' imgName ' now ~~~']);
    % end
    

    function cellMesh = setCellMesh( cellList, cellListN, num)
    ...
    % mesh, meshOut, meshMid, gridLen, gridLenCum
        ...
        for Cell = 1: cellListN
            ...
            mesh = double( cellList.meshData{ num}{ Cell}.mesh); % convert to double
            ...
            if length( mesh) > 1 % sometimes the cell mesh is empty 
                ...
                fprintf( '~~~~~~ Cell #%d is having problem with its mesh! ~~~~~~\n', Cell)
            end
        end
    end


    function cellIsolated = markIsoCells( goodCellFlag, cellListN, cellMesh, imgPath, imgName)
    % Mark the cells as isolated good (1), or non-isolated bad (0)
        ...
        if logical( goodCellFlag) % flag set in autoSportNorm: autorun or manually choose good cells
            fprintf( '    %s  -  You have assumed all cells are good, so I will run by myself\n', imgName);
        else 
            % plot cell image & mesh
            img = imread( fullfile( imgPath, imgName)); imshow( img, []); 
            set( gcf, 'Position', [1100 200 900 900]), hold on  
            for Cell = 1: cellListN
                meshOut = cellMesh( Cell).meshOut;
                if ~isempty( meshOut)
                    plot( meshOut(:,1), meshOut(:,2), 'g-', 'LineWidth', 0.6)
            ...
            text( 5,13, imgName, 'Color', 'w', 'FontSize', 16)
            
            flag = logical( input('    Hi there, are all cells isolated? Yes(1), No(0): '));
            if ~flag
                tmp = input('    Tell me the bad cell numbers (e.g. 2 5 8):  ', 's');
                badCells = sscanf( tmp, '%f')'; % parses any valid number format
    ...
    function tfGood = getNormPos( tfGood, cellMesh, checkFlag, img)
