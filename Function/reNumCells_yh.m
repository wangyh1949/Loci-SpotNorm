...
%     % stack cellList & cellMesh data for future use
%     cellMeshAll = []; cellListAll = []; savePath = '';
%     for r = 1: R % each movie        
%         [savePath, fileName] = getSavePath( char( dataOrigin(r)), savePath);
%         load( [savePath fileName], 'cellList', 'cellMesh'); % load corresponding cellList data    
%         
%         chosenCell{r} = find( cellNumCount2D( r, :)); % find the good chosed cells for each movie
%         cellMeshAll = [cellMeshAll; cellMesh( chosenCell{r})]; 
%         cellListAll = [cellListAll; [ cellList.meshData{1}{ chosenCell{r}}]']; % add the cells as stucture array
%     end        
%     cellRecord = [ cellstr( dataOrigin), chosenCell]; % fileName + chosenCell
end
