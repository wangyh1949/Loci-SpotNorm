...
Author: Yu-Huan Wang (Kim Lab at UIUC) - yuhuanw2@illinois.edu
    ...
    ~~~~~~ adapted from plot_subMSD_spotNorm_fit.m ~~~~~~

Description: This script plot loci subpopulation's MSD binned by signal
amplitude
...
fitR = 1:30;        fitTxt = sprintf( '%d:%d fit', min( fitR), max( fitR));
...
for j = plotNum
    ...
    binData = tracksAmp(:,1);  binName = 'amp'; % use amplitude at first frame for binning
    ...
    for k = 1: numel( binList)- 1
        ...
        condTracks = cond & condAmp; % overall flag for good tracks
