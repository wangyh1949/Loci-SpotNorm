...
alphaRef = 0.25;    textRef = sprintf( '\\alpha=%g', alphaRef);
cc = 0;
...
for j = plotNum
    ...
    xlim( limX), ylim( limY)
        ...
        % add reference line: alpha = 0.25
        tRef = time( fitR);
        MSDRef =  4* DFit* 1.3* tRef.^ alphaRef;
        plot( tRef, MSDRef, 'k--', 'LineWidth', 1.5, 'color', [1 1 1]*0.3, 'HandleVisibility','off')
        text( tRef(end)*0.8, MSDRef(end)*1.2, textRef, 'FontSize', 12, 'Color', [1 1 1]*0.3)
end
